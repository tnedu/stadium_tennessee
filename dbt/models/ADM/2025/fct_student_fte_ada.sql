...
with aggregate_before_explosion as (
    select k_student, k_school, k_lea, k_school_calendar, school_year, is_primary_school, entry_date, 
        ...
        sum(is_early_grad_date) over (
            partition by k_student, k_school, k_school_calendar, is_primary_school, entry_date, report_period
        ) as early_grad_days, 
        sum(isa_member) over (
            partition by k_student, k_school, k_school_calendar, is_primary_school, entry_date, report_period
        ) member_days, 
        sum(is_funding_ineligible) over (
            partition by k_student, k_school, k_school_calendar, is_primary_school, entry_date, report_period
        ) as funding_ineligible_days,
        sum(is_absent) over (
            partition by k_student, k_school, k_school_calendar, is_primary_school, entry_date, report_period
        ) as absent_days, 
        sum(is_suspended) over (
            partition by k_student, k_school, k_school_calendar, is_primary_school, entry_date, report_period
        ) as suspended_days, 
...
daily_attendance as (
    select k_student, k_school, k_lea, k_school_calendar, school_year, is_primary_school, entry_date, 
        ...
        case
            ...
            else cast((floor(program_duration / cast(ssd_duration as decimal(20,8)) * 100000.0) / 100000.0) as decimal(12,5))
        end as daily_attendance,
        case
            when isa_member = 0 then 0
            when coalesce(ssd_duration, 0) = 0 then 0
            when coalesce(program_duration, 0) = 0 then 0
            else cast((floor(program_duration / cast(ssd_duration as decimal(20)) * 100000.0) / 100000.0) as decimal(12,5))
        end as daily_attendance_ifa_member,
        case
            when isa_member = 0 or is_absent = 1 or is_suspended = 1 then 0
            when coalesce(ssd_duration, 0) = 0 then 0
            when coalesce(program_duration, 0) = 0 then 0
            else cast((floor(program_duration / cast(total_duration as decimal(20,8)) * 100000.0) / 100000.0) as decimal(12,5))
...
fake_schedule_by_dow as (
    ...
    select k_student, k_school, k_lea, k_school_calendar, school_year, is_primary_school, entry_date, 
        exit_withdraw_date, day_of_week, 

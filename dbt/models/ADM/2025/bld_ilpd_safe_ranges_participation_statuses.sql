...
with ilpd_statuses as (
    select ilpd.k_student, ilpd.k_school, ilpd.school_year, ilpd.tenant_code, ilpd.api_year,
        ...
        coalesce(exploded_status.value:statusEndDate::date, to_date(concat(ilpd.school_year, '-06-30'), 'yyyy-MM-dd')) as status_end_date
    from {{ ref('stg_ef3__student_language_instruction_program_associations') }} ilpd,
    lateral variant_explode(ilpd.v_program_participation_statuses) as exploded_status
...
ordered as (
    select k_student, k_school, school_year, tenant_code, api_year, student_unique_id, ed_org_id,
        status_begin_date, status_end_date,
...
aggregated as (
    select k_student, k_school, school_year, tenant_code, api_year, student_unique_id, ed_org_id,
        min(status_begin_date) as service_begin_date,
        max(status_end_date) as service_end_date
    from island_ids
...
select k_student, k_school, school_year, tenant_code, api_year, student_unique_id, ed_org_id,
    ...
    'programParticipationStatuses' as source_location

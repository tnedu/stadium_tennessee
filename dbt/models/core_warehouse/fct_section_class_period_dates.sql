...
with bell_schedule_periods_dates as (
...
section_cps_dates as (
    select sec.k_course_section, cp.k_class_period, cp.k_school, sec.k_course,
    ...
    from {{ ref('dim_course_section') }} sec
    join {{ ref('dim_session')}} sess
        on sess.k_session = sec.k_session
    join {{ ref('stg_ef3__sections__class_periods') }} scp
        on scp.k_course_section = sec.k_course_section
    join {{ ref('edu_wh', 'dim_class_period') }} cp
        on cp.k_class_period = scp.k_class_period
    join bell_schedule_periods_dates bs
    ...
    where bs.calendar_date >= sess.session_begin_date
        and (sess.session_end_date is null or bs.calendar_date <= sess.session_end_date)
)
select * from section_cps_dates
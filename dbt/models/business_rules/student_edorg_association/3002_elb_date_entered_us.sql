...
errors as (
    select se.k_student, se.k_lea, se.k_school, se.school_year, se.ed_org_id, se.student_unique_id,
        ...
        concat('ELB Student ', 
            ...
            'with LEP codes [L, W, 1, 2, 3, 4, F, N] require Date Entered US on District 
            level Student/EdOrg Associations.') as error,
    ...
    where s.date_entered_us is null
        /* We only want this rule to fire if there exists an enrollment that is non-zero-day early grad and not in service schools. */
        and exists (
            select 1
            from valid_enrollents_minus_zeroday_early_grads_minus_service_sch x

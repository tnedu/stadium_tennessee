...
with q as (
    select distinct
        ...
        lea.lea_id,
        lea.lea_name,
        regexp_replace(right(sc.school_id, 4), '^0+', '') as school_number,
        sc.school_id,
        sc.school_name,
        s.student_unique_id,
        s.state_student_id,
    ...
    from {{ ref('fct_student_school_association') }} ssa
        ...
        join {{ ref('dim_school') }} sc
            on ssa.k_school = sc.k_school
        join {{ ref('dim_lea')}} lea
            on sc.k_lea = lea.k_lea
